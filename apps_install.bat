...
wsl -d test bash -c "touch /tmp/wsl.conf; cat /etc/wsl.conf > /tmp/wsl.conf; sudo rm -rf /etc/wsl.conf; echo >> /tmp/wsl.conf; echo [boot] >> /tmp/wsl.conf; echo systemd=true >> /tmp/wsl.conf; sudo mv /tmp/wsl.conf /etc/wsl.conf"
